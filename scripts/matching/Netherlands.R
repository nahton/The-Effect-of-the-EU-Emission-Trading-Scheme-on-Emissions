...
prtr_nl <- read.csv(repo_path("PRTR Netherlands", "prtr_nl_new.csv"),
                    header = TRUE,
                    sep = ";") |>
  rename(year = jaar,
         facility = bedrijfsnaam,
         id = nic,
         emissions = "emissie.totaal") |>
  select(year, id, facility, emissions) |>
...
pre_2013_NA_entities <- ets_compliance %>%
  filter(year < 2013, substr(installation_id,1,2) == "NL") %>%
...
from_2005_entities <- ets_compliance %>%
  filter(year >= 2005, substr(installation_id,1,2) == "NL") %>%
  group_by(installation_id) %>%
...
list_2005_ets_nl <- setdiff(unique(from_2005_entities$name), unique(post_2012_entities$name))
list_ets_nl <- c(unique(post_2012_entities$name))

matched <- read.csv(repo_path("PRTR Netherlands", "Matched_Facilities.csv"),
                    sep = ";",
                    header = TRUE) |>
  mutate(manual_reject = .data$X == "x") |>
  filter(!.data$manual_reject) |>
  filter(.data$PRTR_Name != "NA") |>
  filter(.data$ETS_Name %in% c(list_2005_ets_nl))

prtr_nl <- left_join(prtr_nl, matched, by = c("facility" = "PRTR_Name"))

prtr_nl <- prtr_nl |> mutate(ets = ifelse(is.na(ETS_Name), 0, 1))


nl_data <- prtr_nl |> select(year, id, facility, ets, emissions, ETS_Name) |> unique() |>
...
match_model <- run_matchit_variant(ets ~ avg, nl_data_0507, default_discard = "none")
...
did_data_nl <- if ("subclass" %in% names(did_data)) {
  mutate(did_data, subclass = paste0("NL", subclass))
} else {
  mutate(did_data, subclass = NA_character_)
}

message(
  "Netherlands matched sample: ", dplyr::n_distinct(did_data_nl$identifiant),
  " installations; ", nrow(did_data_nl), " plant-year rows."
)
